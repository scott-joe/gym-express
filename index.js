...
// Index
app.get('/', (_req, res) => {
  // Pull file list in users directory
  fs.readdir('users', (err, files) => {
    const users = []

    // Read each file and pull data out into it's own
    files.forEach((filename) => {
      // Pull user's first and last name into users list
      fs.readFile(
        path.join(__dirname, 'users', filename),
        fsOpts,
        (err, data) => {
